...
class SimpleTest(TestCase):
    def setUp(self):
        ...
        self.message=Message.objects.create(messagesubject=self.messagesubject)
        self.content=MessageContent.objects.create(message=self.message,language=self.language,title='title',text='text')
        
        user=User.objects.create_user(username='13888888888',password='111111')
        user.save()
        ...
        self.competition=Competition.objects.create(subject='football',title='title',startdate=datetime.date(2014,8,8),enddate=datetime.date(2014,12,12))
        self.player=Player.objects.create(competition=self.competition,sn=1)
        PlayerInfo.objects.create(player=self.player,language=self.language,desc='足球')

        self.notification=Notification.objects.create(title='title',text='text')
        self.extra_no=ExtraNotification.objects.create(user=self.myuser,notification=self.notification)
    ...
    def test_get_language_list(self):
        ...
        self.assertEqual(json.loads(response.content),[{'id':1,'name':'english'}] )
    ...
    def test_get_user_by_uid_or_create(self):
        data=json.dumps({'cellphone':'13888888888','userid':1,'token':'123456'})
        r=self.client.options('/get_user_by_uid_or_create/111111',data=data)
        self.assertEqual(json.loads(r.content),{u'token':u'123456',u'userid':1,u'created': False})
    def test_set_language(self):
        data=json.dumps({'cellphone':'13888888888','userid':1,'token':'123456','languageid':1})
    ...
    def test_getmessages(self):
        data=json.dumps({'cellphone':'13888888888','userid':1,'token':'123456','subject':'football'})
        r=self.client.options('/get_messages',data=data)
        self.assertEqual(r.content,json.dumps([{u'postdate':self.content.postdate.isoformat(),u'title': self.content.title,
                                    u'text': self.content.text,u'min_index':1,u'language_id':1,u'id':1,u'message_id':1}]))
    def test_get_message(self):
        r=self.client.get('/get_message/1')
        self.assertEqual(r.content,json.dumps([{u'postdate':self.content.postdate.isoformat(),u'title': self.content.title,
                                    u'text': self.content.text,u'language_id':1,u'id':1,u'message_id':1}]))
    def test_set_favorite_and_get_favorites(self):
        data=json.dumps({'cellphone':'13888888888','userid':1,'token':'123456','messageid':1,'languageid':1})
    ...
    def test_get_competitions(self):
        ...
        self.assertEqual(r.content,json.dumps([{'id':1,'subject':'football','title':'title','startdate':self.competition.startdate.isoformat(),'enddate':self.competition.enddate.isoformat()}]))
    ...
    def test_register(self):
        r=self.client.post('/register',data={'cellphone':'13666666666','password':'111111'})
        user=MyUser.objects.get(id=2)
        self.assertEqual(user.cellphone,'13666666666')
        self.assertEqual(self.client.login(username='13666666666',password='111111'),True)
    def test_if_cellphone_exist(self):
        r=self.client.get('/if_cellphone_exist/13888888888')
        self.assertEqual(r.content,json.dumps(True))
    def test_login(self):
        r=self.client.post('/login',data={'cellphone':'13888888888','password':'111111'})
        user=MyUser.objects.get(cellphone='13888888888')
        self.assertEqual(r.content,user.token)
    def test_modify_password(self):
        r=self.client.post('/modify_password',data={'cellphone':'13888888888','oldpassword':'111111','newpassword':'222222'})
        self.assertEqual(r.content,json.dumps(True))
    def test_update_user_info(self):
        token=self.client.post('/login',data={'cellphone':'13888888888','password':'111111'}).content
    ...
    def test_invite_and_pass_and_deny(self):
        self.client.post('/register',data={'cellphone':'13666666666','password':'111111'})
        token=self.client.post('/login',data={'cellphone':'13888888888','password':'111111'}).content
        data=json.dumps({'userid':1,'token':token,'target_cellphone':'13666666666'})
